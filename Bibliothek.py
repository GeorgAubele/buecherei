...
# app = Flask(name)
# app.secret_key = b'_5#y2L"F4Q8z\n\xec]/'
# app.config["SQLALCHEMY_DATABASE_URI"] = "mysql://buch:leseratte@localhost/mydatabase"
...
datum_31_dezember = date(aktuelles_jahr, 12, 31)
silvester = date_to_str(datum_31_dezember)
...
window = webview.create_window(
    "Unsere Bibliothek", app, width=1100, height=950)
...
def start_page():
    ...
    if request.method == "POST":
        if "btn_search_last_name" in request.form:
            ...
            search_result = (
                db.session.query(Benutzer).filter_by(
                    BenutzerID=search_id).all()
            )
            # print(search_result)
            flash("Benutzer erfolgreich gesucht.")
            return render_template("index.html", users=search_result)
...
def book_search():
    ...
    if request.method == "POST":
        ...
        if "btn_tags_search" in request.form:
            ...
            search_result = Buecher.query.filter(
                or_(*[Buecher.Schlagworte.ilike(f"%{tag}%")
...
def book_import():
    ...
    if request.method == "POST":
        ...
        if "btn_import_books" in request.form:
            ...
            books = Buecher.query.filter(
                Buecher.ISBN.in_(imported_book_list)).all()
    ...
    if request.method == "POST":
        ...
        if "btn_book_new" in request.form:
            ...
            if book_ISBN in list_of_ISBNs:
                exist_book = db.session.query(
                    Buecher).filter_by(ISBN=book_ISBN).first()
                exist_book.Anzahl += 1
                exist_book.Momentan_vorhanden += 1
                db.session.commit()
...
def lending():
    ...
    if request.method == "POST":
        if "btn_search_last_name" in request.form:
            ...
            searched_book = db.session.query(
                Buecher).filter_by(ISBN=book_ISBN).first()
            sql_query = "SELECT buecher.Titel, ausleihen.Ausleihdatum from buecher INNER JOIN ausleihen ON buecher.BuchID = ausleihen.BuchID INNER JOIN benutzer ON ausleihen.BenutzerID = benutzer.BenutzerID WHERE ausleihen.Rueckgabedatum IS NULL AND benutzer.BenutzerID = :ID"
            result = db.session.execute(text(sql_query), {"ID": a_ID})
            rows = []
    ...
    if request.method == "POST":
        if "new_btn" in request.form:
            ...
            with app.app_context():
                ...
                db.session.commit()
            return render_template("user_management.html", silvester=silvester)
            ...
            flash("Benutzer erfolgreich gesucht.")
            return render_template(
                "user_management.html", users=search_result, silvester=silvester
            )
    ...
    if request.method == "POST":
        if "btn_year_search" in request.form:
            ...
            for row in result:
                data = {
                    "Medium": row[0],
                    "Anzahl_Buecher": row[1],
                    "Anzahl_Ausleihen": row[2]
                }
                rows.append(data)
